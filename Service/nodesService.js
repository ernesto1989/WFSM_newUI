...
const getNodesQuery = "select id as recid,id,scenario_id,node_id,description,max_capacity,min_capacity,current_vol from a01_nodes where scenario_id = ? and region_id = ?";
const selectInsertA01 = "INSERT INTO a01_nodes(scenario_id,id, region_id ,node_id, description, max_capacity, min_capacity, current_vol) SELECT ?, a01.id as id, a01.region_id as region,a01.node_id as node_id,a01.description,a01.max_capacity,a01.min_capacity,a01.current_vol FROM a01_nodes a01 WHERE a01.scenario_id = ? and region_id = ?";
const insertNodeQuery = "Insert into a01_nodes(scenario_id,id, region_id, node_id, description, max_capacity, min_capacity, current_vol) Values (?,?,?,?,?,?,?,?)";
const updateNodeQuery = "Update a01_nodes set node_id = ?, description = ?, max_capacity = ?, min_capacity = ?, current_vol = ? Where scenario_id = ? and id = ? and region_id = ?";
const deleteNodeQuery = "Delete from a01_nodes where scenario_id = ? and id = ? and region_id = ?";
