...
const getFlowsQuery = "SELECT ROW_NUMBER() OVER(PARTITION BY 'scenario_id' ) AS recid, a02.origin,coalesce(a01o.node_id,'IN') as origin_node, a02.destiny, coalesce(a01d.node_id,'OUT') as destiny_node,a02.current_flow,a02.`type`,coalesce(fmax,0) as fmax,coalesce(fmin,0) as fmin FROM a02_flows a02 left join a01_nodes a01o on a01o.scenario_id = a02.scenario_id and a01o.id = a02.origin left join a01_nodes a01d on a01d.scenario_id = a02.scenario_id and a01d.id = a02.destiny WHERE a02.scenario_id = ?";
