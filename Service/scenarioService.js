...
const getScenariosQuery = "SELECT z01.scenario_id,z01.cdate,z01.description,z01.type,z01.capacity_units,z01.time_units,z01a.origin_id as parent_id,(select count(*) from a01_nodes a01 where a01.scenario_id = z01.scenario_id) as nodes,(select count(*) from a02_flows a02 where a02.scenario_id = z01.scenario_id) as flows,COALESCE((SELECT CASE WHEN sol >= 1 then 'Solved' WHEN sol = 0 then 'Unsolved' END AS Solved FROM (SELECT count(*) as sol FROM s02_proposed_flows sd WHERE sd.scenario_id = z01.scenario_id GROUP BY sd.scenario_id) AS sdetail),'Unsolved') AS Status FROM z01_scenarios z01 left join z01_scenarios z01a on z01a.scenario_id = z01.origin_id";
const getScenarioByIdQuery = "SELECT z01.scenario_id,z01.cdate,z01.description,z01.type,z01.capacity_units,z01.time_units,z01a.origin_id as parent_id,(select count(*) from a01_nodes a01 where a01.scenario_id = z01.scenario_id) as nodes,(select count(*) from a02_flows a02 where a02.scenario_id = z01.scenario_id) as flows, z01.recalc_trl, z01.recalc_solution, COALESCE((SELECT CASE WHEN sim >= 1 then 'Simulated' WHEN sim = 0 then 'Uncomputed' END AS simulated FROM (SELECT count(*) as sim FROM a03_time_to_reach_limit a03 WHERE a03.scenario_id = z01.scenario_id GROUP BY a03.scenario_id) AS psol),'Uncomputed') AS Proposed_Solution,COALESCE((SELECT CASE WHEN sol >= 1 then 'Solved' WHEN sol = 0 then 'Unsolved' END AS Solved FROM (SELECT count(*) as sol FROM s02_proposed_flows sd WHERE sd.scenario_id = z01.scenario_id GROUP BY sd.scenario_id) AS sdetail),'Unsolved') AS Optimal_Solution FROM z01_scenarios z01 left join z01_scenarios z01a on z01a.scenario_id = z01.origin_id where z01.scenario_id = ?";
const getScenarioTRLQuery = "select a01.node_id, a03.current_vol, a03.min_vol, a03.max_vol, a03.incoming_flow, a03.outcoming_flow, case when a03.time_to_reach_limit = -1 then 'Stable' else a03.time_to_reach_limit end as time_to_reach_limit from a03_time_to_reach_limit a03 left join a01_nodes a01 on a01.scenario_id = a03.scenario_id and a01.id = a03.node_id where a03.scenario_id = ? "
